...
| tiktok | https://www.tiktok.com | X-Bogus custom signing + SSR | Three blockers: (1) All API requests require `X-Bogus` and `X-Gnarly` query params — client-side computed anti-bot signatures via obfuscated JS (virtual-machine-based). Cannot be reproduced outside browser. (2) Core content (search results, video detail, user profile) served via SSR `__UNIVERSAL_DATA_FOR_REHYDRATION__` (253KB embedded JSON), not separate API calls. (3) Additional `msToken` dynamic signing. Logged-in session captured 5 internal endpoints (notifications, feed, popup) but none are target operations. Needs L3 adapter: `browser_fetch` transport for API calls + `page.evaluate()` for SSR data extraction. |
