...
**Examples**:
- TikTok (X-Bogus + X-Gnarly + msToken on every API request)
- X/Twitter (`x-client-transaction-id` header — required for some endpoints like Followers and SearchTimeline, generated by webpack module 938838 export `jJ`, signature function takes `(host, path, method)`)
