...
Transport: L3 adapter — search and directions use SPA navigation + DOM extraction; place details uses direct fetch to /maps/preview/place.
Search and directions APIs need session-specific tokens the Maps SPA generates during navigation — direct fetch returns incomplete data.
Place details API works via page.evaluate(fetch()) with a minimal `pb` parameter encoding the place ID.
...
Reviews at [6][31][1], hours at [6][203], coordinates at [6][9][2-3], phone at [6][178][0][0].
