...
import { generateDriftReport, generateDriftReportMarkdown, type SiteVerifyResult } from './verify.js'
...
describe('generateDriftReport', () => {
  it('produces correct counts', () => {
    ...
    expect(report.total_sites).toBe(3)
  ...
  it('only includes non-PASS sites', () => {
    ...
    expect(sites.map((s) => s.site)).toEqual(['site-b', 'site-c'])
  })
