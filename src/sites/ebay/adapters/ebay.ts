  ...
  return page.evaluate(`
    (() => {
      ...
      for (const card of cards) {
        ...
        if (!match || match[1] === '123456') continue;
        ...
        const img = card.querySelector('.s-card__image img');
        ...
        results.push({
          ...
          condition: subtitle,
          image,
          link: 'https://www.ebay.com/itm/' + match[1],
        });
      }
...
async function getItemDetail(
  ...
  await page.goto(`https://www.ebay.com/itm/${itemId}`, { waitUntil: 'load', timeout: 30_000 })
  await wait(3000)
  ...
  return page.evaluate(`
    (() => {
      ...
      const sellerPositive = [...(sellerCard?.querySelectorAll('span') || [])].find(s => s.textContent?.includes('positive'))?.textContent?.trim() || '';

      // Store link for seller profile
      ...
      if (ldProduct) {
        ...
        return {
          ...
          images: Array.isArray(ldProduct.image) ? ldProduct.image : [ldProduct.image].filter(Boolean),
          seller: {
            name: sellerName,
          ...
          shipping: g('[data-testid="x-shipping-header"] .ux-textspans') || g('.ux-labels-values--shipping .ux-textspans--BOLD'),
      ...
      return {
        ...
        image: document.querySelector('.ux-image-carousel-item img')?.getAttribute('src') || '',
        seller: {
          name: sellerName,
          feedbackScore: sellerFeedback,
          positivePercent: sellerPositive,
  ...
  return page.evaluate(`
    (() => {
      ...
      if (notFound && notFound.textContent?.includes('not found')) {
        return { error: 'Seller not found', username: '${username}' };
      }

      const card = document.querySelector('.str-seller-card');
      if (!card) return { error: 'Could not load seller profile', username: '${username}' };

      const storeName = document.querySelector('.str-seller-card__store-name h1')?.textContent?.trim() || '';
      const feedbackLink = card.querySelector('.str-seller-card__feedback-link');
      const feedbackText = feedbackLink?.textContent?.trim() || '';
      const fullText = card.textContent?.trim() || '';

      // Parse stats from full text like "99.8% positive feedback59K items sold11K followers"
      ...
      const categories = [...categoryCards].slice(0, 10).map(c => c.textContent?.trim() || '').filter(Boolean);

      return {
        username: '${username}',
        storeName,
        logoUrl: logo?.getAttribute('src') || '',
        positiveFeedback: positiveMatch ? positiveMatch[1] + '%' : feedbackText,
