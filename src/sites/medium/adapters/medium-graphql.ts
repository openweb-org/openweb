...
const TOPIC_LATEST_STORIES_QUERY = `query TopicLatestStorieQuery($tagSlug: String!) {
...
const TOPIC_CURATED_LISTS_QUERY = `query TopicCuratedListQuery($tagSlug: String!) {
  tagFromSlug(tagSlug: $tagSlug) {
...
const TOPIC_WRITERS_QUERY = `query TopicWhoToFollowPubishersQuery($first: Int!, $after: String!, $mode: RecommendedPublishersMode!, $tagSlug: String) {
...
const RECOMMENDED_FEED_QUERY = `query WebInlineRecommendedFeedQuery($forceRank: Boolean, $paging: PagingOptions) {
...
const RECOMMENDED_TAGS_QUERY = `query RightSidebarQuery {
...
const PUBLICATION_POSTS_QUERY = `query PostDetailQuery($postId: ID!) {
...
const POST_CLAPS_QUERY = `query ClapCountQuery($postId: ID!) {
  postResult(id: $postId) {
    ... on Post {
      id
      clapCount
...
const RECOMMENDED_WRITERS_QUERY = `query WhoToFollowModuleQuery {
