...
### Transport
`page` — required because Reuters returns 401 for direct Node.js requests. The adapter's `init()` navigates to reuters.com if the browser tab is on a blank or unrelated page (also recognizes DataDome captcha redirects as valid state). API calls run from within the browser tab using `page.evaluate(fetch(..., {credentials: 'same-origin'}))`. `getArticleDetail` navigates the page to the article URL and extracts content from the Fusion SSR data.
