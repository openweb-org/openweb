...
- **Remaining 6 ops (still failing):**
  - `followUser`, `blockUser` — operation timed out after 45 s. Hypothesis: pair-order asymmetry — the inverse `unfollowUser`/`unblockUser` runs first (alphabetical/pair `order`), which means `followUser` is asked to follow a user the account doesn't currently un-follow. Resolving requires either reordering the pair so the create op runs first, or seeding the target's relationship state.
  - `muteUser`, `unmuteUser` — `page.evaluate: Target page, context or browser has been closed` / `no browser tab open for this site`. The mute action's navigation appears to close the previous page handle. Likely needs a per-op `page_plan` override that opens the target user's profile in a sticky tab.
  - `hideReply`, `unhideReply` — operation timed out. Both need a real reply tweet ID under a tweet the authenticated user owns; the current example fixture targets a stale reply.
- See `doc/todo/write-verify/handoff.md` §3.3 for the full hypothesis list and fix paths.
