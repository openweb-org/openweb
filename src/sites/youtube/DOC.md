...
- **Cross-domain SAPISID cookie scope (2026-04-20)**: Google SSO sets `SAPISID` / `__Secure-1PAPISID` on `.google.com`, **not** on `.youtube.com` — `__Secure-3PAPISID` is the only `*PAPISID` variant that lives on `.youtube.com`. An adapter that calls `page.context().cookies('https://www.youtube.com')` only sees the 3P cookie and silently misses the 1P values. Adapter now queries both domains and emits a multi-prefix `Authorization` header (`SAPISIDHASH ts_h SAPISID1PHASH ts_h SAPISID3PHASH ts_h`) so the server picks whichever prefix matches a cookie it can see in the actual request. **Caveat:** a `*HASH` prefix only validates if the matching cookie is also *sent* with the request. Since `.google.com`-scoped cookies are not attached to `https://www.youtube.com` requests by the browser, only `SAPISID3PHASH` will validate against a profile that lacks 1P cookies on `.youtube.com`. Profiles that don't surface `SID`/`SAPISID` on `.youtube.com` after sign-in still fail with HTTP 401 on write ops — this is a profile-state issue, not an adapter scope issue.
