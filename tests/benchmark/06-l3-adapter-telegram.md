...
1. `openweb telegram` — check readiness (adapter (L3), Requires browser: yes, Requires login: yes)
2. `openweb telegram getDialogs` — inspect operation
3. `openweb telegram exec getDialogs '{}' --cdp-endpoint http://localhost:9222` — execute
